...
function buildTable($results, $columns, $table = NULL, $actions = false)
{
    ...
    foreach ($results as $row) {
        ...
        if ($actions) {
            $str .= "<td><button class='btn btn-outline-success m-1' style='border-width:2px; font-weight: bold' type='submit' name='operation' value='goto_update' >Edit</button>
            <button class='btn btn-outline-danger m-1' style='border-width:2px; font-weight: bold'  type='submit' name='operation' value='delete'>Delete</button></td>";
        }
        $str .= '</form></tr>';
    }
    return $str . '</table>';
        ...
        $str .= <<<EOD
            <tr>
                <form method="POST" action="opmanager.php">
                    ...
                    <td> {$row['esame']} </td>
                    <td> <input type='hidden' name='descrizione' value='{$row['descrizione']}'> {$row['descrizione']} </td>
                    <td> <input type='hidden' name='regimeprivato' value='{$regimePrivato}'> {$row['regimeprivato']} </td>
                    <td><button type="submit" class="btn btn-1 border-2 m-4 rounded-pill fw-bold btn-sm" style="color:rgb(255, 255, 255); ">Crea una prenotazione</button></td>
                </form>
            </tr>
        EOD;
    }
...
{  
    ...
    return <<<EOD
        ...
        <div class="row g-3 align-items-center py-1 fw-bold">
            <div class="col-auto">
                <label class="col-form-label" style="inline-size: 100px;"for="{$name}">{$name}:</label>
            </div>
            <div class="col-auto">
...
function buildInputPassword($name, $minsize, $maxsize, $required, $editable = true, $value = NULL)
{
    ...
    return <<<EOD
         <div class="row g-3 align-items-center pb-3 fw-bold">
            <div class="col-auto">
                <label class="col-form-label" style="inline-size: 100px;" for="{$name}">{$name}:</label>
            </div>
            <div class="col-auto">
...
{
    if (!$visualizedOptions) { // se $visualizedOptions non è specificato 
        $visualizedOptions = $options; // viene usato $options
    }
...
function getRequired($r)  //se $r è vero restituisce "required" - - - se $r è falso restituisce ""
{
    return ($r) ? "required" : "";
}
